...
function brandForText(brand: string, isLight: boolean): string {
  ...
  switch (brand) {
    case C1: return "#03796a";
    case C2: return "#03796a";
    case C3: return "#00505a";
...
function Tag({ label }: { label: string }) {
  return <div style={{ fontSize:11,fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",color:"var(--text-muted)",marginBottom:5 }}>{label}</div>;
}
function STitle({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize:"clamp(15px,1.8vw,18px)",fontWeight:800,color:"var(--text-primary)",marginBottom:"clamp(12px,1.5vw,18px)" }}>{children}</div>;
}
function Pill({ label, color=C1 }: { label:string;color?:string }) {
  ...
  return <span style={{ display:"inline-flex",alignItems:"center",padding:"5px 12px",borderRadius:20,fontSize:13,fontWeight:600,background:`${color}12`,color:textColor,border:`0.5px solid ${color}30`,margin:"3px" }}>{label}</span>;
}
function Step({ num, title, desc }: { num:number;title:string;desc:string }) {
  return (
    <div style={{ display:"flex",gap:14,alignItems:"flex-start",paddingBottom:14 }}>
      ...
      <div style={{ flex:1 }}>
        <div style={{ fontSize:"clamp(13px,1.5vw,15px)",fontWeight:700,color:"var(--text-primary)",marginBottom:4 }}>{title}</div>
        <div style={{ fontSize:"clamp(12px,1.3vw,14px)",color:"var(--text-muted)",lineHeight:1.7 }}>{desc}</div>
      </div>
    </div>
  );
}
...
const DATA = {
  ar: {
    ...
    models:[
      {name:"YOLOv26",role:"كشف الطائرات",desc:"مُدرَّب على صور الطائرات المسيّرة. يعمل 25+ إطار/ثانية.",color:C1},
    ...
    ai:["YOLOv26","ByteTrack","Qwen2.5-3B","QLoRA 4-bit","Ollama","Hugging Face"],
    ...
    arch:[
      ...
      {l:"Detection",d:"YOLOv26 + ByteTrack في workers متوازية لكل كاميرا"},
    ],
  },
  en: {
    ...
    stats:[{v:"3,026",l:"Attack Records"},{v:"3",l:"Drones"},{v:"2",l:"Languages"}],
    tabs:["Overview","Models","Technology"],
    prob_tag:"The Problem",prob_title:"Why was this built?",
    prob_pts:["Drones are a low-cost weapon and hard to detect with traditional radar","Used in swarms to overwhelm air defense systems","Target critical infrastructure: oil, power, water","Operators need to decide in seconds — not minutes"],
    ...
    steps:[
      ...
      {title:"YOLOv26 — Detection",desc:"Analyzes every frame: Shahed-136, Orlan-10, DJI, aircraft, bird, helicopter"},
      {title:"ByteTrack — Tracking",desc:"Assigns each target a unique ID and tracks it across frames"},
      {title:"Geographic Analysis",desc:"Converts pixel to GPS, determines heading, and computes speed and ETA to the nearest sensitive site"},
      {title:"Threat Assessment",desc:"Scores threat level and suggests the optimal intercept point"},
      {title:"Operator",desc:"Monitors cameras, responds to alarms, and prepares reports"},
    ],
    ...
    data_items:[
      ...
      {l:"1,950 Q&A pairs",d:"Bilingual Q&A set for training the AI assistant"},
    ],
    models_tag:"AI Models",models_title:"Models Used",
    models:[
      {name:"YOLOv26",role:"Drone Detection",desc:"Trained on drone images. Runs at 25+ FPS in real time.",color:C1},
    ...
    arch:[
      ...
      {l:"Detection",d:"YOLOv26 + ByteTrack in parallel workers per camera"},
    ],
  },
};
...
export function About() {
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:"clamp(12px,1.8vw,18px)" }} data-mount>
      ...
      <Card>
        <div style={{ display:"flex",alignItems:"flex-start",gap:18,flexWrap:"wrap" }}>
          ...
          <div style={{
            ...
            <img src="/logo.svg" alt="رقيب" width="46" height="46" style={{ display:"block" }}/>
          </div>
          <div style={{ flex:1,minWidth:200 }}>
            ...
            <div style={
              isLight
                ? { fontSize:"clamp(17px,2.2vw,22px)", fontWeight:800, marginBottom:10, color:"#0b2422" }
                : { fontSize:"clamp(17px,2.2vw,22px)", fontWeight:800, marginBottom:10, background:`linear-gradient(90deg,${C1},${C3})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }
            }>
              {d.hero_title}
            </div>
            <div style={{ fontSize:"clamp(13px,1.5vw,15px)",color:"var(--text-muted)",lineHeight:1.75 }}>{d.hero_desc}</div>
          </div>
        </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginTop:20 }}>
          {d.stats.map(s=>(
            <div key={s.l} style={{ background:"var(--bg-elevated)",border:"1px solid var(--border-subtle)",borderRadius:12,padding:"14px 16px",textAlign:"center" }}>
              ...
              <div style={
                isLight
                  ? { fontSize:"clamp(20px,2.5vw,26px)", fontWeight:800, color:"#0b2422", lineHeight:1 }
                  : { fontSize:"clamp(20px,2.5vw,26px)", fontWeight:800, background:`linear-gradient(135deg,${C1},${C3})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text", lineHeight:1 }
              }>{s.v}</div>
              <div style={{ fontSize:"clamp(10px,1.1vw,12px)",color:"var(--text-muted)",marginTop:5,textTransform:"uppercase",letterSpacing:"0.10em" }}>{s.l}</div>
            </div>
          ))}
        </div>
      </Card>
      ...
      {/* Tabs */}
      <div style={{ display:"flex",gap:3,padding:4,background:"var(--bg-elevated)",border:"1px solid var(--border-subtle)",borderRadius:13 }}>
        {d.tabs.map((t,i)=>(
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:"clamp(12px,1.8vw,18px)" }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
              {d.prob_pts.map((p,i)=>(
                <div key={i} style={{ display:"flex",alignItems:"flex-start",gap:10 }}>
                  ...
                  <div style={{ fontSize:"clamp(13px,1.5vw,15px)",color:"var(--text-muted)",lineHeight:1.7 }}>{p}</div>
                </div>
              ))}
            </div>
          </Card>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:10 }}>
            {d.data_items.map((di,i)=>(
              <Card key={i}>
                <div style={{ fontSize:"clamp(12px,1.4vw,14px)",fontWeight:700,color:brandForText(C1,isLight),marginBottom:8 }}>{di.l}</div>
                <div style={{ fontSize:"clamp(12px,1.3vw,14px)",color:"var(--text-muted)",lineHeight:1.7 }}>{di.d}</div>
              </Card>
            ))}
          </div>
        </div>
          ...
          {d.models.map((m,i)=>(
            <Card key={i}>
              <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:12 }}>
                ...
                <div>
                  <div style={{ fontSize:"clamp(14px,1.6vw,16px)",fontWeight:800,color:brandForText(m.color,isLight) }}>{m.name}</div>
                  <div style={{ fontSize:"clamp(9px,1vw,11px)",color:"var(--text-muted)",textTransform:"uppercase",letterSpacing:"0.12em" }}>{m.role}</div>
                </div>
              </div>
              <div style={{ fontSize:"clamp(12px,1.3vw,14px)",color:"var(--text-muted)",lineHeight:1.75 }}>{m.desc}</div>
            </Card>
          ))}
        </div>
      )}
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
          ...
          <Card>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
              {d.arch.map((a,i)=>(
                <div key={i} style={{ display:"flex",gap:14,alignItems:"flex-start",padding:"12px 14px",background:"var(--bg-elevated)",borderRadius:10,border:"1px solid var(--border-subtle)" }}>
                  <div style={{ fontSize:"clamp(12px,1.3vw,14px)",fontWeight:700,color:brandForText(C1,isLight),minWidth:90,flexShrink:0 }}>{a.l}</div>
                  <div style={{ fontSize:"clamp(12px,1.3vw,14px)",color:"var(--text-muted)",lineHeight:1.7 }}>{a.d}</div>
                </div>
              ))}
            </div>
          </Card>
