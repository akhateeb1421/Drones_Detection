...
function Chevron({ expanded, rtl }: { expanded: boolean; rtl: boolean }) {
  const deg = rtl ? (expanded ? 180 : 0) : (expanded ? 0 : 180);
  return (
...
function TopBar({ isAr }: { isAr: boolean }) {
  const { t } = useTranslation();
  const { role } = useRole();
  const { user } = useAuth();
  ...
  return (
    <div className="page-title-row">
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
        {/* Role is server-verified now — the pill is an indicator, not a
            toggle. Switching roles means signing in as a different account. */}
        <div className="role-pill-container" title={user?.username}>
          <span className="role-pill role-pill-active">
            {isAdmin ? t("auth.role_admin","Admin") : t("auth.role_viewer","Operator")}
          </span>
          <span style={{ fontSize:12, fontWeight:600, color:"var(--muted-foreground)", padding:"0 10px", whiteSpace:"nowrap" }}>
            {user?.username}
          </span>
        </div>
        <button
...
function Shell() {
  ...
  // While an alarm banner is up (fixed, full-width, above everything),
  // shift the sidebar and workspace down so the alert is fully clear of
  // the logo and page title instead of covering them.
  const { latest: activeAlarm } = useAlarmsContext();
  const ALARM_H = 64;
    ...
    <div style={{ minHeight:"100vh", position:"relative", direction: isAr ? "rtl" : "ltr", backgroundColor:"var(--background)", color:"var(--foreground)", overflowX:"hidden" }}>
      ...
      */}
      <aside className="sidebar glass" data-expanded={String(expanded)}
        style={{ top: activeAlarm ? `calc(var(--sb-mg) + ${ALARM_H}px)` : undefined, transition: "top 300ms cubic-bezier(0.22,1,0.36,1), width 260ms" }}>
        ...
        {/* Brand logo — inline SVG driven by the theme tokens, so it
            recolors with light/dark automatically. No background tile:
            the mark sits directly on the glass sidebar. */}
        <div className="sidebar-logo" style={{ flexShrink:0, background:"transparent", boxShadow:"none" }}>
          <BrandLogo size={34}/>
        </div>
        ...
        <div style={{ borderTop:"1px solid var(--border)", width:"100%" }}>
          <div style={{ padding:"10px 0 6px", display:"flex", flexDirection:"column", alignItems: expanded ? "stretch" : "center", gap:6, paddingInline: expanded ? 12 : 0 }}>
            {/* 1. Signed-in identity (server-verified) */}
            <div className="sidebar-ctrl" style={{ cursor:"default" }} title={user?.username}>
              <span style={{
              ...
              <span className="sidebar-ctrl-label">{user?.username}</span>
            </div>
            {/* 2. Sign out — everyone signs in via the login page now */}
            <button
      ...
      <main style={{
        ...
        paddingInlineEnd: 32, paddingTop: activeAlarm ? 40 + ALARM_H : 40, paddingBottom: 40,
        transition:"padding-inline-start 280ms cubic-bezier(0.22,1,0.36,1), padding-top 300ms cubic-bezier(0.22,1,0.36,1)",
...
function Gate() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", backgroundColor:"var(--background)", color:"var(--muted-foreground)" }}>
        ...
      </div>
    );
  }
